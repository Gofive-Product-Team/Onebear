...
### Custom Fields Moved to Settings (updated)

Customer Custom Field management (previously shown as part of CRM page) has been relocated to **Settings → CRM → Custom Fields**.

**Rationale:** Custom field definitions are schema/configuration decisions made by Super Admins, not routine CRM operations. Moving to Settings keeps the CRM page focused on customer records while giving admins a dedicated configuration surface.

**What remains in CRM:**
- Customer list (table/card view), segment filtering, search
- Customer profile sidebar (with custom fields rendered and editable per customer)
- Merge/deduplication workflow
- Assign customers, bulk follow-up

**What moved to Settings:**
- Defining/editing/deleting custom field types (schema management)
- Setting field scope (all channels / LINE only / Facebook only / specific channels)
- Setting required/optional for fields

**Cross-feature impact:** When a Super Admin adds a custom field in Settings → Custom Fields, it appears immediately in CRM customer edit profiles and in the Onboarding flow's customer data collection step.
