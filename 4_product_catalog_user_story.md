...
These changes were added to the React prototype based on product review:

### Categories Tab (added)
- Top-level tab "หมวดหมู่" added alongside "สินค้า" and "Variant Templates"
- Displays product categories in a 2-column grid
- Each category card shows: name, icon emoji, product count, Edit/Delete actions
- Inline edit (click pencil → name field becomes editable)
- Delete with confirmation prompt
- "สร้างหมวดหมู่ใหม่" button opens modal with name + icon picker
- Category names match dropdown in product create/edit form

### Variant Templates Tab (added)
- Top-level tab "Variant Templates" for reusable variant sets
- Templates: ขนาดเสื้อ (XS/S/M/L/XL/XXL), สีเสื้อพื้นฐาน (ดำ/ขาว/เทา/กรม), ขนาด+สี (combined), ขนาดครีม (ขนาดเล็ก/กลาง/ใหญ่)
- Each template shows expandable combination preview
- Templates selectable during product variant setup
- "สร้าง Template ใหม่" button for custom variant sets

### ProductDetailPanel — Basic Info Tab (added)
- "ข้อมูลสินค้า" is now the DEFAULT first tab when opening product detail panel
- Contains: live image preview (h-40, object-cover), URL input field, amber warning banner if image missing
- All editable fields: Name, Category dropdown, Price, Stock, Pre-order toggle, Status pill buttons (เปิดขาย / ปิดขาย), Description
- "บันทึกข้อมูล" button triggers save
- Previously the panel opened directly on Variants tab — changed to Info first for better UX flow

### Filter Bar Enhancements
- "เรียง OOS ก่อน" (Sort out-of-stock first) toggle button added to filter bar
- Out-of-stock products bubble to top of list when enabled
- Effective stock calculation: `effectiveStock ?? 1` treated as available if null/undefined

### AI Suggestion Confidence Badges
- Confidence badges are now color-coded:
  - ≥ 80%: green (bg-success/15 text-success)
  - ≥ 60%: yellow (bg-warning/15 text-warning)
  - < 60%: red (bg-error/15 text-error)
- AI suggestion cards show: original price (strikethrough), suggested price, discount %

### Upsell — Per-product Price Cap Field
- `upsellMaxPrice` editable field added to Upsell tab
- Admin sets per-product max upsell price threshold
- AI enforces this cap when scoring suggestions

### Offline-safe Product Images
- Product thumbnails use SVG data URI placeholder when image URL is missing or fails to load
- No dependency on external image CDNs (works offline/in dev without internet)
