...
function ServicesTab() {
  ...
  function handleSave(form: ServiceFormData) {
    ...
    // Try API mutation but also add locally for instant feedback
...
function AvailabilityTab() {
  ...
  return (
    <div className="space-y-5">
      ...
      <Button onClick={handleSaveAll} className="w-full sm:w-auto">บันทึกทั้งหมด</Button>
    </div>
  )
}
...
export function BookingsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-5">
        ...
        {mainTabs.map((tab) => (
          <button
            ...
            className={cn(
              ...
              mainTab === tab.key ? 'bg-primary text-white shadow-sm' : 'text-t2 hover:bg-bg-hover',
            )}
          >
            {tab.icon}
            {tab.label}
      ...
      {/* Tab content */}
      {mainTab === 'bookings' && <BookingsListTab />}
