...
**Status**: Updated — Config moved to Settings (v2)
...
### Bookings Page (simplified)
- Bookings page (`/bookings`) now shows **appointments list only**
- Removed "บริการ" and "ตารางงาน" tabs from Bookings page
- Page subtitle: "จัดการนัดหมายและการจองทั้งหมด"

### Configuration Moved to Settings
All booking configuration is now under **Settings > Booking** (Messaging group):

**บริการ tab:**
- Add/edit/delete services (name, duration, price, active toggle)
- Table view: ชื่อบริการ, เวลา, ราคา, พนักงาน, สถานะ, จัดการ
- "+ เพิ่มบริการ" modal

**ตารางงาน tab:**
- Working hours per day (toggle on/off, from/to time)
- Buffer time between appointments (0–30 min)
- Blocked dates / วันหยุด (add date + reason)
- "บันทึกทั้งหมด" save button

### Rationale
Booking configuration (services, hours, holidays) is admin-level setup that belongs in Settings alongside other operational configs (Auto-reply, Follow-up, Payment, etc.). The Bookings page focuses on operational day-to-day appointment management.
